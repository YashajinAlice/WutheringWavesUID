...
class RoleDetailData(BaseModel):
    ...
    def get_skill_list(self):
        sort = ["常态攻击", "共鸣技能", "共鸣解放", "变奏技能", "共鸣回路"]
        return sorted(
            [skill for skill in self.skillList if skill.skill.type != "延奏技能"],
            key=lambda x: sort.index(x.skill.type) if x.skill.type in sort else 999,
        )
