...
async def echo_data_to_cost(
    char_id, mainProps_first, index, cost4_counter=0
) -> tuple[int, int]:
    """
    ...
    attribute_value = mainProps_first[0]["attributeValue"]

    # 驗證屬性值是否有效
    if not attribute_value or attribute_value.strip() in [".", "-", ""]:
        logger.warning(f"[鸣潮]無效的屬性值: {attribute_value}, 使用默認值")
        return ECHO_ID_COST_ONE, 1  # 返回默認值

    try:
        value = float(attribute_value.strip("%"))
    except ValueError as e:
        logger.error(f"[鸣潮]無法轉換屬性值為浮點數: {attribute_value}, 錯誤: {e}")
        return ECHO_ID_COST_ONE, 1  # 返回默認值
    ...
    def select_cost4_id():
        ...
        if len(cost_4_id_list) >= 2:
            ...
            if (
                needId and needId == cost_4_id_list[used_idx]
            ):  # 如果指定了id且在列表之中
