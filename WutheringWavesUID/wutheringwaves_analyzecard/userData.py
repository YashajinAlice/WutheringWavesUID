...
async def save_card_dict_to_json(bot: Bot, ev: Event, result_dict: Dict):
    ...
    try:
        ...
        if char_id is None:
            await bot.send(
                f"[鸣潮]识别结果为角色'{char_name_print}'不存在\n", at_sender
            )
...
async def compare_update_card_info(uid, waves_data):
    ...
    try:
        ...
        # 驗證必要數據結構
        if not waves_data.get("role") or not existing_data.get("role"):
            logger.warning(f" [鸣潮][dc卡片识别] 角色數據結構不完整，跳過比較")
            return waves_data
    except Exception as e:
        logger.error(f" [鸣潮][dc卡片识别] 數據比較過程中發生錯誤：{e}")
        return waves_data
    ...
    # 驗證武器數據結構
    if (
        not waves_data.get("weaponData")
        or not existing_data.get("weaponData")
        or not waves_data["weaponData"].get("weapon")
        or not existing_data["weaponData"].get("weapon")
    ):
        logger.warning(f" [鸣潮][dc卡片识别] 武器數據結構不完整，跳過武器比較")
        return waves_data
    ...
    # 確保兩個技能列表都存在且長度一致
    if (
        waves_data.get("skillList")
        and existing_data.get("skillList")
        and len(waves_data["skillList"]) == len(existing_data["skillList"])
    ):

        for i in range(len(waves_data["skillList"])):
            # 确保技能等级是整数类型
        ...
        logger.warning(f" [鸣潮][dc卡片识别] 技能列表數據不一致，跳過技能等級比較")
