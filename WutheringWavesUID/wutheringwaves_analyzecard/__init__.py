...
from ..utils.cooldown_manager import analyze_cooldown_manager
...
async def analyze_card(bot: Bot, ev: Event):
    ...
    is_subscriber = analyze_cooldown_manager.is_subscriber(user_id)
    can_use, remaining_time = analyze_cooldown_manager.can_use(user_id, is_subscriber)

    if not can_use:
        cooldown_message = analyze_cooldown_manager.get_cooldown_message(user_id)
        return await bot.send(f"[鸣潮] {cooldown_message}\n", at_sender)
    ...
    if ev.text.strip():
        ...
        success = await async_ocr(bot, ev)
        if success:
            analyze_cooldown_manager.mark_success(user_id)
        else:
            analyze_cooldown_manager.mark_failure(user_id)
        return
