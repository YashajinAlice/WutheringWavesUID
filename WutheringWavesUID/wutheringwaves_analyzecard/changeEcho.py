...
async def change_echo(bot: Bot, ev: Event):
    ...
    if not bool_get:
        return await bot.send(
            f"[鸣潮] 用户{uid}数据不存在，请先使用【{PREFIX}分析】上传{char_name_print}角色数据\n",
            at_sender,
        )
    ...
    if not char_id:
        return await bot.send(
            f"[鸣潮] 角色{char_name_print}不存在，请先使用【{PREFIX}分析】上传角色数据\n",
            at_sender,
        )
    char_name_print = re.sub(
        r"[^\u4e00-\u9fa5A-Za-z0-9\s]", "", roleName
    )  # 删除"漂泊者·衍射"的符号

    bool_change, waves_data = await change_sonata_and_first_echo(
        bot, char_id, sonata, phantom, old_data
    )
    if not bool_change or isinstance(waves_data, str):
        return await bot.send(
            f"[鸣潮] 修改角色{char_name_print}数据失败，{waves_data}\n", at_sender
        )
...
async def change_sonata_and_first_echo(
    ...
    if sonata_a:
        ...
        if len(ECHO) != len(char_data["phantomData"]["equipPhantomList"]):
            return (
                False,
                f"套装数 {len(ECHO)}与角色声骸数 {len(char_data['phantomData']['equipPhantomList'])}不一致",
            )
    ...
    if phantom_a:
        ...
        # 检查是否有可用选项
        if not options:
            return False, f"套装{sonata}没有可用的声骸选项，请检查配置"
        ...
        TEXT_GET_RESP = (
            ...
            + "\n\n注意："
            + "\n• 1cost和3cost声骸为随机指派，可能与实际不符"
            + "\n• 4cost声骸为准确识别，基于主词条判断"
            + "\n• 随机指派不影响声骸评分，仅影响显示名称"
        )
        ...
        if (
            ...
            if 0 <= choice < len(flat_choices):
                ...
                same_cost_ids = [
                    echo_id
                    for g in phantom_id_list_groups
                    if g["cost"] == target_cost
                    for echo_id in g["list"]
                ]
                ...
                for echo in char_data["phantomData"]["equipPhantomList"]:
                    if int(echo["cost"]) == target_cost:
                        if first_change_bool:
                            ...
                            echo["phantomProp"]["name"] = phantom_id_to_phantom_name(
                                selected_id
                            )
                            first_change_bool = False
                        else:
                            if other_phantoms:
                                echo["phantomProp"]["phantomId"] = other_phantoms[
                                    0
                                ]  # 取第一个不同的元素
                                echo["phantomProp"]["name"] = (
                                    phantom_id_to_phantom_name(other_phantoms[0])
                                )
                            else:
                                # 如果没有其他选项，保持原样
                                pass

                logger.info(f"[鸣潮] 修改cost{target_cost}声骸id为:{selected_id}")
            else:
