...
async def change_sonata_and_first_echo(
    bot: Bot, char_id: int, sonata_a: str | None, phantom_a: bool, role_data: dict
):
    # 检查角色是否存在
    ...
    if sonata_a:
        ...
        if len(ECHO) != len(char_data["phantomData"]["equipPhantomList"]):
            return (
                False,
                f"套装数 {len(ECHO)}与角色声骸数 {len(char_data['phantomData']['equipPhantomList'])}不一致，请考虑在套装名后紧跟声骸数量(无数字则默认为5)",
            )
                        ...
                        if int(echo["cost"]) == target_cost:
                            if first_change_bool:
                                ...
                                echo["phantomProp"]["name"] = (
                                    phantom_id_to_phantom_name(selected_id)
                                )
