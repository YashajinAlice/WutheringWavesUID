...
async def get_global_session():
    ...
    if OCR_SESSION is None or OCR_SESSION.closed:
        ...
        connector = aiohttp.TCPConnector(
            limit=10, keepalive_timeout=60  # 空闲连接保留 60 秒
        )
...
async def check_ocr_link_accessible(key="helloworld") -> bool:
    ...
    try:
        ...
        async with session.get(
            url, data=payload, timeout=ClientTimeout(total=10)
        ) as response:
            data = await response.json()
            logger.debug(
                f"[鸣潮]OCR.space示例链接访问成功，状态码为 {response.status}\n内容：{data}"
            )
...
async def check_ocr_engine_accessible() -> int:
    ...
    try:
        ...
        async with session.get(url, timeout=ClientTimeout(total=10)) as response:
            ...
            # 定位目标表格
            target_table = soup.find("h4", string="API Access Points").find_next(
                "table"
            )
            ...
            for row in target_table.find_all("tr"):
                ...
                if len(cells) >= 4 and "Free OCR API" in cells[0].text:
                    ...
                    logger.info(
                        f"[鸣潮] OcrEngine_1:{status_1}, OcrEngine_2:{status_2}"
                    )
                    if status_2 == "UP":
                        return 2
                    elif status_1 == "UP":
...
async def async_ocr(bot: Bot, ev: Event):
    ...
    if not ocr_results or ocr_results[0]["error"]:
        logger.warning("[鸣潮]OCRspace辨識異常！请检查服务器网络是否正常。")
        return await bot.send(
            "[鸣潮]OCRspace辨識異常！目前伺服器正在維護，請稍作等候約5-10分鐘。\n",
            at_sender,
        )
...
async def images_ocrspace(api_key, engine_num, cropped_images):
    ...
    results = await asyncio.gather(
        *(process_with_semaphore(task, semaphore) for task in tasks)
    )
...
async def fetch_ocr_result(session, url, payload):
    ...
    try:
        async with session.post(
            url, data=payload, timeout=10
        ) as response:  # ✅ 添加单次请求超时
            # 检查HTTP状态码
...
async def ocr_results_to_dict(chain_num, ocr_results):
        ...
        if first_result["error"] is None:
            ...
            for line in lines:
                ...
                if level_match and not final_result["角色信息"].get("等级"):
                    final_result["角色信息"]["等级"] = min(
                        int(level_match.group(1)), 90
                    )  # 最大等级为90
    ...
    if len(ocr_results) > 1 and ocr_results[1]["error"] is None:
        ...
        for line in lines:
            ...
            line_clean = re.sub(
                r"[^0-9\u4e00-\u9fa5\s]", "", line_clean
            )  # 先删除非数字中英文的符号, 匹配“源能臂铠·测肆”
            line_clean = re.sub(r"\s+", " ", line_clean).strip()  # 再合并多余空白
        ...
        if valid_entries:
            ...
            for entry in valid_entries[2:7]:
                equipment["subProps"].append(
                    {"attributeName": entry[0], "attributeValue": entry[1]}
                )
...
async def which_char(bot: Bot, ev: Event, char: str):
    ...
    try:
        ...
        await bot.send(
            f"[鸣潮] 选择多次出错，已自动使用 {default_name}\n", at_sender=at_sender
        )
        return default_name, default_id
    except Exception:
        default_name, default_id = flat_choices[0] if flat_choices else (char, None)
        await bot.send(
            f"[鸣潮] 选择出错，已自动使用 {default_name}\n", at_sender=at_sender
        )
        return default_name, default_id
