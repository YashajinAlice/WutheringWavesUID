...
async def get_all_rank_info(
    ...
    # 如果指定了鏈數過濾，只保留指定鏈數的數據
    if chain_filter is not None:
        rankInfoList = [
            rank_info for rank_info in rankInfoList if rank_info.chain == chain_filter
        ]
...
async def draw_bot_rank_img_by_chain(
    bot: Bot, ev: Event, char: str, rank_type: str, chain: int
) -> Union[str, bytes]:
    """按鏈數顯示bot排行"""
    char_id = char_name_to_char_id(char)
    if not char_id:
    ...
    start_time = time.time()
    logger.info(f"[get_bot_rank_info_for_user_by_chain] start: {start_time}")
    # 获取bot里的所有拥有该角色人的数据
    users = await WavesBind.get_all_data()
    ...
    if not users:
        msg = []
        msg.append(f"[鸣潮] bot内暂无【{char}】{chain}鏈面板")
        msg.append(f"请使用【{PREFIX}刷新面板】后再使用此功能！")
        if tokenLimitFlag:
            msg.append(
                f"当前排行开启了登录验证，请使用命令【{PREFIX}登录】登录后此功能！"
    ...
    title_name = f"{char_name}{chain}鏈{rank_type}bot排行"
    title_draw.text((540, 265), f"{title_name}", "black", waves_font_30, "lm")
    ...
    # 备注
    if rank_type == "伤害":
        temp_notes = f"排行标准：以期望伤害（计算暴击率的伤害，不代表实际伤害) 为排序的排名 - 仅显示{chain}鏈"
    else:
        temp_notes = f"排行标准：以声骸分数（声骸评分高，不代表实际伤害高) 为排序的排名 - 仅显示{chain}鏈"
    card_img_draw.text((700, 500), f"{temp_notes}", SPECIAL_GOLD, waves_font_16, "lm")
    ...
    logger.info(f"[get_rank_info_for_user_by_chain] end: {time.time() - start_time}")
    return card_img
